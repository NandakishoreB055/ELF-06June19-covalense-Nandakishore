...
//			req.getRequestDispatcher("cookiesdiabled.html").include(req, resp);
//			return;
//		}

		EmployeeDAO dao = EmployeeDAOFactory.getInstance();
		EmployeeData bean = dao.getEmployeeInfo(req.getParameter("eid"));
...
//			Cookie sessionCookie = new Cookie("JSESSION", session.getId());// non-persistent cookie from browser
//			sessionCookie.setMaxAge(7 * 24 * 60);// make browser cookie as persistent cookie
			...
			out.print(
					"<div class=\"search col-3\"><form action =\"./search\"><input type=\"text\" placeholder=\"Search\" name=\"search\"/></form></div>        ");
			out.print(
			...
			resp.sendRedirect("http://localhost/Empmngmnt/index.html?error=invalid");
		}
	}
}
