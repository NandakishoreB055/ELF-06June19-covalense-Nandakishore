...
public class TopperInClass {
	...
	public static void main(String[] args) {
		...
		List<Student> femaleStudentList = arrayList.stream().sorted(c).filter(s -> s.gender.equalsIgnoreCase("female"))
				.collect(Collectors.toList());
		log.info(" Female Topper  :"
				+ arrayList.stream().sorted(c).filter(s -> s.gender.equalsIgnoreCase("female")).max(c).get());
		Student femaleTopper = femaleStudentList.stream().max(c).get();
		List<Student> maleStudentList = arrayList.stream().sorted(c).filter(s -> s.gender.equalsIgnoreCase("male"))
				.collect(Collectors.toList());
